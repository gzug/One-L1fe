...
import { loadPanels, savePanels } from '../../../v1-marathon/src/data/bloodStorage';
import type { BloodMarker, BloodPanel } from '../../../v1-marathon/src/data/bloodStorage';
import { getMarkerContext } from '../../../v1-marathon/src/data/markerContext';
...
export function BloodResultsScreenV2({ onClose }: Props) {
    ...
    <View style={s.safeArea}>
      <View style={[s.header, { paddingTop: ANDROID_TOP_INSET + spacing.sm }]}>
        ...
        >
          ...
          <Text style={[s.backLabel, { color: colors.textMuted }]}>Home</Text>
        </Pressable>
        <View style={s.headerCenter}>
          <View style={s.headerBrandRow}>
            <BrandMarkV2 size={24} />
            <Text style={s.headerTitle}>Test Results</Text>
          </View>
          <Text style={s.headerSub}>
            {viewMode === 'compare'
              ? 'Compare panels over time without changing score logic.'
              : 'Review imported panels and keep edits local to this device.'}
          </Text>
        </View>
      ...
      >
        <View style={s.container}>
          ...
          <Text style={[s.footer, { color: colors.textSubtle }]}>
            Informational only. Test results stay local in this prototype and are not used for diagnosis or treatment guidance.
          </Text>
        </View>
      </ScrollView>
    </View>
...
function createStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    headerCenter:   { flex: 1, gap: 4, paddingTop: 2 },
    headerBrandRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
    headerTitle:    { color: colors.text, fontSize: typography.subtitle, fontWeight: '800', fontFamily: 'BrandHeading' },
    headerSub:      { color: colors.textSubtle, fontSize: typography.micro, lineHeight: lineHeights.caption, paddingRight: spacing.sm },
    saveBtn:        { minHeight: 38, paddingHorizontal: spacing.md, paddingVertical: spacing.sm - 2, borderRadius: radius.pill, backgroundColor: colors.accent, justifyContent: 'center' },
    saveBtnDisabled:{ opacity: 0.5 },
    saveBtnText:    { color: '#FFFFFF', fontSize: typography.bodySmall, fontWeight: '700' },
    savedIndicator: { minHeight: 38, flexDirection: 'row', alignItems: 'center', gap: 3, paddingHorizontal: spacing.sm },
    ...
    infoBanner:     { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.xs, paddingHorizontal: layout.screenPaddingH, paddingVertical: spacing.sm, borderBottomWidth: StyleSheet.hairlineWidth },
    ...
    card:           { borderRadius: radius.lg, backgroundColor: colors.surface, borderWidth: StyleSheet.hairlineWidth, overflow: 'hidden' },
    ...
    uploadBtn:      { flex: 1, minHeight: 46, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.xs, borderWidth: 1, borderRadius: radius.md, paddingVertical: spacing.sm + 2, backgroundColor: colors.surface },
    ...
    // CompareCard
    compareCard:    { borderRadius: radius.lg, backgroundColor: colors.surface, borderWidth: StyleSheet.hairlineWidth, overflow: 'hidden' },
